...
CREATE SCHEMA IF NOT EXISTS audit_manucci;
SET search_path TO audit_manucci, public;
REVOKE ALL ON SCHEMA audit_manucci FROM public;

COMMENT ON SCHEMA audit_manucci IS 'Out-of-table audit/history logging tables and trigger functions';
...
CREATE INDEX IF NOT EXISTS logged_actions_relid_idx ON logged_actions(relid);
CREATE INDEX IF NOT EXISTS logged_actions_action_tstamp_tx_stm_idx ON logged_actions(action_tstamp_stm);
CREATE INDEX IF NOT EXISTS logged_actions_action_idx ON logged_actions(action);
...
$body$
    LANGUAGE plpgsql
    SECURITY DEFINER
    SET search_path = audit_manucci, pg_catalog, public;
